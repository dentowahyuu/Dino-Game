...
func _process(_delta):
	if game_running:
		...
		$Dino.position.x += int(speed)
		$Camera2D.position.x += int(speed)
...
func show_score():
	$HUD.get_node("ScoreLabel").text = "SCORE: " + str(float(score) / float(SCORE_MODIFIER))

func check_high_score():
	if score > high_score:
		high_score = score
		$HUD.get_node("HighScoreLabel").text = "HIGH SCORE: " + str(float(high_score) / float(SCORE_MODIFIER))
...
func _on_restart_button_pressed():
	$CharacterBody2D.restart_game()  # Memanggil fungsi restart pada karakter
